...
public class RingNote : MonoBehaviour
{
    ...
    private CheckTiming checkTiming;
    ...
    void Start()
    {
        ...
        checkTiming = target.GetComponent<CheckTiming>();        //判定ラインはどこ?
    ...
    void Update()
    {
        ...
        transform.localScale = new Vector3(scale2,1,scale2);
        ...
        if (statusflag == 0)
        {
            if (this.gameObject.transform.localScale.x <= badscale/2)            //大きさがBADエリアと同じになったらリストに追加.Pro Builderとスケールの縮尺が違うらしい。
            {
        ...
        {
            if (this.gameObject.transform.localScale.x <= goodscale/2)            //大きさがGOODエリアと同じになったらリストに追加
            {
        ...
        {
            if (this.gameObject.transform.localScale.x <= perfectscale/2 + 0.01f)            //大きさがPerfectエリアと同じになったらリストに追加
            {
        ...
        {
            if (this.gameObject.transform.localScale.x <= perfectscale / 2 - 0.1f)            //大きさがPerfectエリア外にでたらリストから削除
            {
                checkTiming.Timing["perfect"].RemoveAt(0);          //Listの先頭行を削除して前詰めする
                checkTiming.Timing["good"].RemoveAt(0);          //Listの先頭行を削除して前詰めする
